...
for f = factors'
    ...
    caption = sprintf(['Kruskal Wallis non-parametric (distribution free) test comparing '...
        ...
        '(meanRank$_{%s}$=%4.2f, meanRank$_{%s}$=%4.2f)'],...
...
caption = sprintf('Comparing activation and connectivity Match effects (meanrank$_{activation}$=%2.3f,meanrank$_{connectivity}$=%2.3f)',stats.meanranks(1),stats.meanranks(2))   ; 
tbl = c3nl.roundTable(cell2table(tbl(2:end,:),'VariableNames',matlab.lang.makeValidName(tbl(1,:))),4);
...
caption = sprintf('Comparing activation and connectivity MisMatch effects (meanrank$_{activation}$=%2.3f,meanrank$_{connectivity}$=%2.3f)',stats.meanranks(1),stats.meanranks(2))   ; 
tbl = c3nl.roundTable(cell2table(tbl(2:end,:),'VariableNames',matlab.lang.makeValidName(tbl(1,:))),4);
...
for f = factors'
    for m = metric'
        for s = set'
            ...
            [p,SEM,ts,CI] =get.empiricalP(M_perf.Null(ix),M_perf.test(ix),19);
            [D,sd,m1,m2,sd1,sd2] = get.mannwhitneyR(M_perf.test(ix),M_perf.Null(ix));
            Tg = [Tg;table(m,f,s,m1,sd1,D,p,CI)];
        end
    end
end
